...
if randcaption == "騙你":
    ...
    if random.randint(1, 2) > 0:
        with open("DONT CLICK ME.vbs", "w") as f:
            f.writelines([
                'X=MsgBox("The virus has been unleashed",0+16,"INSEYNIAN VIRUS")\n',
                'X=MsgBox("This is fake btw, but please save all of your documents before continuing",0+64,"INSEYNIAN VIRUS")\n',
                "\n"
...
def main_game(loc):
    ...
    while True:
        ...
        if timer >= 12*(int(clock.get_fps()))*60*dt:
            day = not day
            timer = 0
        if not day and daylight_alpha < 150:
            daylight_alpha += 5*dt
        elif day and daylight_alpha > 0:
            daylight_alpha -= 5*dt
