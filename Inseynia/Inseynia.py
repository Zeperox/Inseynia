...
class Player:
    ...
    def move(self, movement, tiles, dt):
        ...
        if pressed_keys[keys["Up"]]:
            movement[1] -= round(self.stats["Speed"]*dt)
        if pressed_keys[keys["Down"]]:
            movement[1] += round(self.stats["Speed"]*dt)
        if pressed_keys[keys["Right"]]:
            movement[0] += round(self.stats["Speed"]*dt)
        ...
        return movement, collision_types
    ...
    def collide_world(self, movement, map_rects):
        collision_types = {"Top": False, "Bottom": False, "Left": False, "Right": False}
        hit_list = self.collision_test(map_rects)
        for tile in hit_list:
            if movement[0] > 0:
                self.rect.right = tile.left
                collision_types["Right"] = True
            ...
            try:
                ...
                if not item in Player.equipment:
                    ...
                    if ind == 0:
                        ...
                        Player.stats["Speed"] = Player.stats["Speed"]-((Player.stats["Speed"]*equipment[1][item]["Speed"])/100)
                else:
                    self.inv = True
                    return
            except:
...
def story():
    ...
    def redraw():
        win.fill((0,0,0))
        try:
            page.render(win, sprites_Story_Photoes[f"S{pages[pg_num]+1}"])
        except:
            page.render(win)
        brightness()
...
def main_game(loc):
    ...
    timer = 0
    day = True
    daylight = pygame.Surface((Width, Height))
    daylight.fill((0,0,0))
    daylight_alpha = 0
    ...
    def redraw():
        ...
        daylight.set_alpha(daylight_alpha)
        win.blit(daylight, (0,0))
    ...
    while True:
        ...
        if not "Always Day" in rdata[loc]["extras"] or not "Always Night" in rdata[loc]["extras"]:
            timer += 1
            if timer >= 12*(int(clock.get_fps()))*60*dt:
                day = not day
                timer = 0
            if day and daylight_alpha < 150:
                daylight_alpha += 5*dt
            elif not day and daylight_alpha > 0:
                daylight_alpha -= 5*dt
        ...
        movement = [0, 0]
        movement = player.move(movement, [], dt)
...
def dev_room():
    ...
    def room():
        ...
        while True:
            ...
            movement = [0, 0]
            movement, collision_types = player.move(movement, map.tile_rects, dt)
